class Maps < Formula
  ...
  def install
    ...
    libexec.install "cli${SCALA_SUFFIX}-${MAPS_CLI_VERSION}.jar"
    ...
    (bin/"maps").write <<~EOS
      ...
      exec "§{JAVA_HOME}/bin/java" §{ALLOW_DEEP_REFLECTION:-} -Dfile.encoding=UTF8 -cp "#{libexec}/cli${SCALA_SUFFIX}-${MAPS_CLI_VERSION}.jar" "com.here.platform.cli.MapsMain" "§@"
    EOS
  end
